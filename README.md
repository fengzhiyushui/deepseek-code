...
- **自动多智能体编排** —— 复杂任务(多文件 / 多子目标)由路由器自动判别并拆成子任务、子代理执行(**无依赖且文件不重叠的子任务在隔离工作区并行、改完合并**)、**两级独立审核**后汇总;失败/不完整会**多回合自适应**(看审核摘要重规划或继续派发,`maxRounds` 可配)、审批中断**可续跑**(不重规划);简单任务零开销走原路。统一入口、确定性编排、成本闸常开,**无需开关**。
