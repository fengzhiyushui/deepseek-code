...
- **Automatic multi-agent orchestration** — complex requests (multiple files / sub-goals) are routed automatically: split into sub-tasks, executed by sequential sub-agents, **independently reviewed (two-level)**, then synthesized; simple requests take the fast path with zero overhead. Unified entry, deterministic orchestration, always-on cost gates — **no toggle needed**.
